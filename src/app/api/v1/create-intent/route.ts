...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const cleanUpiLink = `upi://pay?pa=${payeeVpa}&pn=${cleanName}&am=${parseFloat(amount).toFixed(2)}&cu=INR&tr=${encodeURIComponent(finalOrderId)}&tn=${encodeURIComponent(finalOrderId)}&tid=${encodeURIComponent(finalOrderId)}&mc=5411&oobe=fos123&qrst=stk&ver=01&mode=01`;
    const cleanPaytmLink = `paytmmp://cash_wallet?pa=${payeeVpa}&pn=${encodeURIComponent(merchantName)}&am=${parseFloat(amount).toFixed(2)}&cu=INR&tn=${encodeURIComponent(finalOrderId)}&featuretype=money_transfer`;
