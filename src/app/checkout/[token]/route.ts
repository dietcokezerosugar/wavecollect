...
export async function GET(req: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  const paytmIntent = generatePaytmIntent({
    ...
    isAndroid: true // SSR defaults to Android style, client-side will intercept if needed or we let mobile redirect handle it. Wait, SSR is tricky for OS detection.
  });
...
// In case we want to dynamically replace URLs on the client side based on OS
if (isIOS) {
  // Update links for iOS if we rendered Android as default in SSR
  const merchantUpi = "${merchantUpi}";
  const merchantName = "${merchantName}";
  const amount = "${amount}";
  const referenceId = "${referenceId}";
  
  // Replace links natively if needed
  // ... Note: For now we let the default links be rendered, but we can dynamically patch them if we prefer pure client generation later.
}
